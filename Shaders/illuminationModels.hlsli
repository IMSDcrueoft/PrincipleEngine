...
/*
	将法线图采样的法线转换到世界空间
*/
float3 NormalMapToWorld(float3 normalMapSample, float3 unitNormalW, float3 tangentW)
{
	// 将采样出的法线数据从[0,1]转化到[-1,1]范围
...
float CalcPbAttenuation(float distance, float falloffStart)
{
	/* 平方反比定律，小心分母为0 */
	return saturate((falloffStart*falloffStart)/(distance*distance+0.001f));
}
/*	
	Blinn-Phong光照模型
